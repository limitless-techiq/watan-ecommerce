...
class FrontendController extends Controller
{
    ...
    public function productCategory($slug, $id, $type = 'physical')
    {
        ...
        $title = ucfirst(get_translation($category->name)) . ucfirst(get_translation( " all products"));
    ...
    public function productDetails($slug, $id, $campSlug = null)
    {
        ...
        $product = Product::with([
            'campaigns' => function ($q) use ($campaignSlug) {
    ...
    public function shippingMethod()
    {
        ...
        $shippingMethod = ShippingDelivery::with(['method'])->where('name', $search)->where(
            'status',
            1
        )->first();
