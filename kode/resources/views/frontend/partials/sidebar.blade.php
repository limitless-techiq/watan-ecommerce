<div class="offcanvas offcanvas-start d-lg-none" tabindex="-1" id="menu-sidebar" aria-labelledby="menu-sidebar">
    <div class="sidebar-top">
        <div class="sidebar-logo">
            <a href="{{route('home')}}">
                <img src="{{ show_image('assets/images/backend/logoIcon/'.$general->site_logo, file_path()['site_logo']['size']) }}"
                    alt="site_logo.png">
            </a>
        </div>
        ...
        <div class="sidebar-middel-container">
                    ...
                    <div class="Dropdown">
                        ...
                        <ul class="dropdown__list">
                            ...
                            <li class="dropdown__list-item" data-value="{{$language->code}}">
                                <a href="{{route('language.change',$language->code)}}" class="notify-item language"
                ...
                <div class="currency-dropdown">
                    ...
                    <div class="Dropdown">
                        ...
                        <ul class="dropdown__list">
                            ...
                            <li class="chanage_currency dropdown__list-item {{$site_currency->id == $currency->id ? 'dropdown__list-item_active' :'' }}  "
                                data-value="{{$currency->id}}">{{$currency->name}}</li>
                            @endforeach
                        </ul>
                        <input class="dropdown__input_hidden" type="text" name="select-category" value="" />
                    </div>
            ...
            <div class="sidebar-middel">
                <div class="mobile-category-container">
                    <button class="w-100 mobile-categoryBtn" type="button" data-bs-toggle="collapse"
                        data-bs-target="#mobileCategoryBtn" aria-expanded="false" aria-controls="mobileCategoryBtn">
                        ...
                        <ul class="browse-categories-items">
                            ...
                            <li class="browse-categories-item flex-column">
                                <a href="{{route('category.product', [make_slug(@get_translation($category->name)), $category->id])}}"
                                    @if($category->parent_count > 0) data-bs-toggle="collapse"
                                    data-bs-target="#mobileCategory-{{$loop->index}}" aria-expanded="false"
                                    aria-controls="mobileCategory-{{$loop->index}}" @endif >
                                    <div>
                                        <span>
                                            <img src="{{show_image(file_path()['category']['path'].'/'.$category->image_icon,file_path()['category']['size'])}}"
                                                alt="{{$category->image_icon}}">
                                        </span>
                                ...
                                <div class="collapse mobilecategories-dropdown" id="mobileCategory-{{$loop->index}}">
                                    <ul class="categories-dropdown-items">
                                        ...
                                        <li class="categories-dropdown-item"><a
                                                href="{{route('category.product', [make_slug(@get_translation($category->name)), $category->id])}}">
                                                {{@get_translation($childCat->name)}} <i
                    ...
                    <ul class="sidebar-nav">
                        <li class="sidebar-nav-item">
                            ...
                            <a href="{{url($menu->url)}}" class="sidebar-nav-link ">
                                <span>
                                    <small class="mobile-nav-icon"><img class="rounded-circle "
                                            src="{{show_image(file_path()['menu']['path'].'/'.$menu->image,file_path()['menu']['size'])}}"
                                            alt="{{$menu->name}}"> </small>
                    ...
                    <ul class="sidebar-nav">
                        ...
                        <li class="sidebar-nav-item">
                            <a href="{{'user.track.order'}}" class="sidebar-nav-link">
                                <span>
                                    <small class="mobile-nav-icon"><i
                                            class="fa-solid fa-location-crosshairs"></i></small>
...
<div class="offcanvas offcanvas-start d-lg-none" tabindex="-1" id="filter-sidebar" aria-labelledby="filter-sidebar">
    <div class="sidebar-top">
        ...
        <button type="button" class="sidebar-closer" data-bs-dismiss="offcanvas" aria-label="Close"><i
                    ...
                    <div class=" filter-accordion">
                        <div class="py-4 border-bottom">
                            ...
                            <ul class="list-unstyled mb-0 mt-3 filter-list">
                                ...
                                <li>
                                    <a href="{{route('category.product', [make_slug(@get_translation($category->name)), $category->id])}}"
                                        class="d-flex py-1 align-items-center position-relative">
                                        <div class="flex-grow-1">
                        ...
                        <div class="py-4 border-bottom">
                            ...
                            <form action="{{ route(Route::currentRouteName(), Route::current()->parameters()) }}"
                                ...
                                <div class="range-slider mb-4">
                                    @php
                                    $search_min = session()->get('search_min') ? session()->get('search_min') :
                                    round(short_amount($general->search_min));
                                    $search_max = session()->get('search_max') ? session()->get('search_max') :
                                    round(short_amount($general->search_max));
                                    @endphp
                                    <div class="slider-area">
                                        <div id="responsive-range" class="slider"></div>
                                    </div>
